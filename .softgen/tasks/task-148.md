...
## Checklist
- [ ] `activity_log` table with columns: actor_user_id, actor_email, actor_type (user/admin/system/api), action (string), entity_type, entity_id, diff (jsonb before/after), metadata (jsonb for free-form context like IP / user-agent / request path), created_at, indexed on (entity_type, entity_id) and (created_at DESC)
- [ ] RLS: super-admin reads everything; regular users read only rows where actor_user_id = auth.uid() OR entity_type = 'client' and entity_id = their client
- [ ] `log_change_generic()` SECURITY DEFINER trigger function — parameterized by table name, diffs only changed columns, writes to activity_log
...
- [ ] App-layer helper `logActivity({ action, entity, before, after, metadata })` importable from `@/lib/activity-log` for use in API routes
- [ ] Helper captures request IP + user-agent from NextApiRequest headers automatically
