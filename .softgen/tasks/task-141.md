...
## Checklist
- [ ] Add `min="0"` to every numeric price/stock/weight/dimension input in `BasicEditor.tsx`, `PricingTaxTab.tsx`, `InventoryShippingTab.tsx`, `ProductQuickEdit.tsx`, and `VariationEditDialog.tsx` (variants have the same fields)
- [ ] On change of these inputs, clamp negative values to 0 (defensive — spinner arrows bypass `min` in some browsers)
- [ ] In `BasicEditor.tsx`: disable Publish button when `status === "publish"` AND no price set AND free-product flag is off; show a muted inline hint ("Add a price or mark as free product to publish")
- [ ] Verify submit path: `formToWooPayload` should never serialize negative numerics — add a guard in `productEditService.ts` to floor numbers at 0
