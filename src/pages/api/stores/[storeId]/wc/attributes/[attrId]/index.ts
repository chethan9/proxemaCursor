...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (req.method === "PUT") {
      const data = await wooRequest(store, "PUT", `products/attributes/${attrId}`, req.body);
      return res.status(200).json(data);
    }
    if (req.method === "DELETE") {
      const data = await wooRequest(store, "DELETE", `products/attributes/${attrId}?force=true`);
      return res.status(200).json(data);
    }
