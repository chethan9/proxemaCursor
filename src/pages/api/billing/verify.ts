...
import { insertSubscriptionEvent } from "@/services/subscriptionService";
  ...
  try {
    ...
    if (st.status === "paid") {
      ...
      await supabaseAdmin.from("subscriptions").update({
        ...
        renewal_mode: "manual", auto_renew_disabled_reason: "tokenization_not_implemented",
      ...
      await insertSubscriptionEvent(subscriptionId, "payment_succeeded", sub.status, "active", {
        ...
        amountMinor: st.amountMinor, currency: st.currency,
      });
