...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const { store_id, limit = "50", offset = "0", status, customer_id } = req.query;
    ...
    let query = supabase.from("orders").select("*", { count: "exact" }).in("store_id", storeIds);
    if (store_id && typeof store_id === "string" && storeIds.includes(store_id)) query = query.eq("store_id", store_id);
    if (status && typeof status === "string") query = query.eq("status", status);
    if (customer_id && typeof customer_id === "string") query = query.eq("customer_id", parseInt(customer_id));

    const limitNum = Math.min(parseInt(limit as string) || 50, 200);
    const offsetNum = parseInt(offset as string) || 0;
    const { data, count, error } = await query.range(offsetNum, offsetNum + limitNum - 1).order("date_created", { ascending: false });
