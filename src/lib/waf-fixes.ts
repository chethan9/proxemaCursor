...
const FIXES: Record<BlockingService, WafFix> = {
  cloudflare: {
    ...
    copyableExpression: '(http.request.uri.path contains "/wp-json/wc/") or (http.request.uri.path contains "/wp-json/wp/v2/media") or (http.user_agent contains "Proxima-Sync")',
    copyableExpressionLabel: "Cloudflare expression",
    adminMessage: `Hi,

Our WooCommerce integration (Proxima) is being blocked by Cloudflare's Managed Challenge on this site. Could you please add the following Custom WAF Rule in your Cloudflare dashboard?

1. Go to Security → WAF → Custom rules → Create rule
2. Name: "Allow WooCommerce API Integration"
3. Expression (paste exactly):
(http.request.uri.path contains "/wp-json/wc/") or (http.request.uri.path contains "/wp-json/wp/v2/media") or (http.user_agent contains "Proxima-Sync")
4. Action: Skip → All managed rules, Bot Fight Mode
5. Deploy

This only whitelists our API path and our identifiable User-Agent (Proxima-Sync/1.0) — the rest of your site stays fully protected.
...
Thanks!`,
  ...
  sucuri: {
    ...
    steps: [
      ...
      "Add User-Agent: Proxima-Sync/1.0",
      "Optionally, whitelist the URL path /wp-json/wc/* under Whitelist URLs.",
      "Save and wait up to 2 minutes for the rule to propagate.",
    ],
    adminMessage: `Hi,

Sucuri Firewall is blocking our WooCommerce integration (Proxima). Please whitelist the following in your Sucuri dashboard:

User-Agent: Proxima-Sync/1.0
Path: /wp-json/wc/*
...
Thanks!`,
  ...
  wordfence: {
    ...
    steps: [
      ...
      "Under 'Allowlisted Services', add User-Agent: Proxima-Sync/1.0",
      "Save changes.",
    ],
    adminMessage: `Hi,

WordFence is limiting access for our WooCommerce integration (Proxima). Please allowlist us:

1. WordPress admin → WordFence → Tools → Allowlisted URLs
2. Add pattern: /wp-json/wc/*
3. Under 'Allowlisted Services' add User-Agent: Proxima-Sync/1.0
4. Save
...
AWS WAF is blocking our WooCommerce integration (Proxima). Please add a rule:
...
Thanks!`,
  ...
  modsecurity: {
    ...
    steps: [
      ...
      "Alternatively, ask your host to whitelist User-Agent 'Proxima-Sync'.",
      "If using Apache directly, add to .htaccess: SecRuleRemoveByTag 'OWASP_CRS/WEB_ATTACK/SQL_INJECTION' in the wp-json location.",
    ],
    adminMessage: `Hi,

The server's ModSecurity firewall is blocking our WooCommerce integration (Proxima) with a 403/406 error on /wp-json/wc/ requests.

Please contact your hosting provider's support and ask them to:
1. Whitelist /wp-json/wc/* for this domain
2. Or whitelist User-Agent: Proxima-Sync/1.0
...
Thanks!`,
  ...
  unknown: {
    ...
    steps: [
      ...
      "Check your server access logs for the User-Agent 'Proxima-Sync/1.0' — the log entry usually shows which rule rejected the request.",
      "Allowlist the /wp-json/wc/* path or the User-Agent 'Proxima-Sync/1.0' wherever firewalls or security plugins are configured.",
      "If you use Cloudflare, Sucuri, or WordFence, enable the corresponding rule from their dashboards.",
    ],
    adminMessage: `Hi,

Our WooCommerce integration (Proxima) is receiving 403 responses from this site, but we cannot automatically identify which security layer is blocking us.

Please check:
1. Active WordPress security plugins (WordFence, iThemes, etc.)
2. Hosting-level firewall (ModSecurity, Imunify360)
3. Server access logs for User-Agent "Proxima-Sync/1.0"
...
export function getFixForService(service: BlockingService): WafFix {
  return FIXES[service] || FIXES.unknown;
}

export function getAllFixes(): WafFix[] {
